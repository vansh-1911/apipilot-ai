...
export async function generateDocumentation(specId: string): Promise<GenerationResult> {
  try {
    ...
    // The client-side AI integration is optional. When no browser-safe key is
    // configured, persist useful deterministic documentation instead of leaving
    // the specification stuck in a failed state.
    if (!OPENROUTER_API_KEY) {
      const fallback = buildFallbackDocumentation(spec, endpoints);
      const { error: fallbackError } = await supabase.from("generated_docs").insert({
        spec_id: specId,
        overview: fallback.overview,
        auth_guide: fallback.auth_guide,
        quick_start: fallback.quick_start,
        best_practices: fallback.best_practices,
        full_markdown: fallback.full_markdown,
      });
